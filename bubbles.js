...
// ─────────────────────────────────────────────────────────────────────────────
// LOCAL BUBBLE CSV LOADER
// Reads saved bubble CSV directly — no server needed.
// Expected columns: time, open, spot_close, ratio, strike
// ─────────────────────────────────────────────────────────────────────────────
function loadLocalBubbles(input) {
  const file = input.files[0];
  if (!file) return;
  const st = document.getElementById('local-bubble-status');
  ...
  reader.onload = e => {
    try {
      ...
      if (lines.length < 2) { st.textContent = '⚠ Empty file'; return; }

      const headers  = lines[0].split(',').map(h => h.trim().replace(/^"|"$/g,'').toLowerCase());
      const col      = h => headers.indexOf(h);
      const iTime    = col('time'), iOpen = col('open'), iRatio = col('ratio');
      const iSpot    = col('spot_close'), iStrike = col('strike');

      if (iTime < 0 || iOpen < 0 || iRatio < 0) {
        st.textContent = '⚠ Missing columns (need: time, open, ratio)'; return;
      }

      // Derive default optType from filename
      const fileType = file.name.toLowerCase().includes('pe') ? 'PE' : 'CE';

      let pushed = 0;
      lines.slice(1).forEach(line => {
        const cols     = line.split(',');
        const time     = parseInt(cols[iTime]);
        const open     = parseFloat(cols[iOpen]);
        const ratio    = parseFloat(cols[iRatio]);
        if (isNaN(time) || isNaN(open) || isNaN(ratio)) return;

        const spotClose = iSpot   >= 0 ? parseFloat(cols[iSpot])                          : open;
        const strike    = iStrike >= 0 ? (cols[iStrike]||'').trim().replace(/^"|"$/g,'') : '';
        const optType   = strike.toLowerCase().startsWith('pe') ? 'PE'
                        : strike.toLowerCase().startsWith('ce') ? 'CE'
                        : fileType;

        if (BUB.items.length >= BUB.MAX) BUB.items.shift();
        BUB.items.push({
          time,
          chartTime: (selIv===60||selIv===300||selIv===900) ? Math.floor(time/selIv)*selIv : time,
          open,
          spotClose: isNaN(spotClose) ? open : spotClose,
          ratio,
          optType,
          strike,
          spotDelta: 0,
